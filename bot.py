...
async def load_photo(message: types.Message, state: FSMContext):
    ...
    await FSMAdmin.next()
    await sql_part.sql_add_travel(state)
    await state.finish()
...
# TODO: есои значение столбца amount в таблице client_travel равен значению количеству записавшихся на путешествие,
#  при нажатии на путешествие выводить сообщение что "Мест нет"

# TODO: если полная запись, вывести сообщение админу "Полная запись"

# TODO: если кто-то отменил запись, или забронировал, также вывести запись админу и указать сколько мест свободно

# TODO: каждая запрос на запись/удаление/изменение к БД должен проверяться на корректное исполнение

# TODO: реализовать рандомные фразы бота с одинаковым смылом (чтобы на одни и теже обращения он отвечал по разному)

"""проверки"""
# TODO: исключить возможномть записи на путешествие которое уже прошло, например если его забыли удалить (например,
#  из 10 записалось 7, а записаться можно)
